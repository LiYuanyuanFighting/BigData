...
public class SparkDriverStationTime {
	...
	public static void main(String[] args) {
		...
		JavaPairRDD<StationTime, Count> stationPairRDD = readingsRDD.mapToPair(new StationTimePairStationTIme());
		System.out.println("stationPairRDD:" + stationPairRDD.count());
		// Compute the number of total and critical readings for each station
		JavaPairRDD<StationTime, Count> stationTotalPairRDD = stationPairRDD.reduceByKey(new Sum());
		System.out.println("stationTotalPairRDD:" + stationTotalPairRDD.count());
		// Compute criticality
		JavaPairRDD<StationTime, Double> stationPercent = stationTotalPairRDD.mapValues(new Percentage()); 
		System.out.println("stationPercent:" + stationPercent.count());
		// Select stationTime with percentage > threshold
		JavaPairRDD<StationTime, Double> selectedStationPercent = stationPercent.filter(new CriticalPercent(threshold));
		...
		JavaPairRDD<String, timeDateCri> changedPair= selectedStationPercent.mapToPair(new changePairStationTime());
		System.out.println("changedPair:" + changedPair.count() + changedPair.values());
